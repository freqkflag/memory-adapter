...
export async function summarizeEmotionalDrift(vectors, lastN) {
    ...
    const lines = [];
    lines.push(`Emotional drift over last ${drift.count} events (from ${new Date(drift.startTimestamp).toISOString()} to ${new Date(drift.endTimestamp).toISOString()}):`);
    for (const [axis, delta] of Object.entries(drift.axisDeltas)) {
        ...
        lines.push(`- Axis '${axis}' ${direction} by ${delta.toFixed(3)} overall (avg change per step: ${avg.toFixed(3)}).`);
    }
    if (lines.length === 1) {
