...
function inferCapability(goal) {
    ...
    if (g.includes("summarize") || g.includes("summary"))
        return "summary";
    if (g.includes("analyze") || g.includes("analysis"))
        return "analysis";
    if (g.includes("reflect") || g.includes("reflection"))
        return "reflection";
...
function inferDomain(goal) {
    const g = goal.toLowerCase();
    if (g.includes("identity") || g.includes("self"))
        return "identity";
    if (g.includes("timeline") || g.includes("history"))
        return "timeline";
    if (g.includes("project"))
        return "projects";
    if (g.includes("emotion") || g.includes("mood") || g.includes("feeling"))
        return "emotion";
    return undefined;
}
...
export async function executePlan(plan, tools) {
    const results = [];
